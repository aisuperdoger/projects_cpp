 ...
 **/
ssize_t Buffer::readFd(int fd, int *saveErrno) // 读fd，即将数据写入buffer_中
{
    ...
    */
    ...
    // 第一块缓冲区，指向可写空间
    vec[0].iov_base = begin() + writerIndex_; // iov_base指向空间的首地址
    ...
    const int iovcnt = (writable < sizeof(extrabuf)) ? 2 : 1; // 2代表使用两个缓冲区，而不是选择第二个缓冲区
    const ssize_t n = ::readv(fd, vec, iovcnt); // 将内核缓冲区的数据放入vec中

    if (n < 0)
    {
        *saveErrno = errno; // 保存errno
    }
