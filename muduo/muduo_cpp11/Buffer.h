...
public:
    ...
    void retrieve(size_t len) // 从缓冲区中取走len长度的数据
    {
    ...
    std::string retrieveAllAsString() { return retrieveAsString(readableBytes()); } // 从缓冲区中取出所有的数据内容
...
private:
    ...
    void makeSpace(size_t len)
    {
        ...
        else // 这里说明 len <= xxx + writer 把reader搬到从xxx开始 使得xxx后面是一段连续空间
        {
            ...
            std::copy(begin() + readerIndex_,
                      begin() + writerIndex_,  // 把readerIndex_到writerIndex_的数据拷贝到begin+kCheapPrepend起始处
                      begin() + kCheapPrepend);
            readerIndex_ = kCheapPrepend;
    ...
    size_t readerIndex_;   // [readIndex, writeIndex)代表buffer中数据的内容。
    size_t writerIndex_;
};
