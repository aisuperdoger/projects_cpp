...
vector<string> getAllfile(string path, int op) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (!(filename[0] == 'm' && filename[1] == 'r' && filename[len - oplen - 1] == '-'))
            continue;
...
// vector中每个元素的形式为"abc 11111";  返回一个vector，每个元素的形式为"abc 11111";
vector<KeyValue> Myshuffle(int reduceTaskNum) {
    ...
    for (int i = 0; i < filename.size(); i++) {
        ...
        KeyValue kv = getContent(text);  // 获取文件名和文件内容
        string context = kv.value;
        vector<string> retStr = split(context, ' ');
        str.insert(str.end(), retStr.begin(), retStr.end());  // 在str后插入整个retStr
    }
    ...
    sort(retKvs.begin(), retKvs.end(), [](KeyValue& kv1, KeyValue& kv2) {
        return kv1.key < kv2.key;  // 按单词顺序排序？
    });
    return retKvs;
}
