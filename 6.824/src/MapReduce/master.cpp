...
class Master {
   ...
   private:
    ...
    unordered_map<int, int>
        finishedReduceTask;   // 存放所有完成的reduce任务对应的reduce编号
...
void* Master::waitMapTask(void* arg) {
    ...
    pthread_t tid;
    char op = 'm';
    pthread_create(&tid, NULL, waitTime,
                   &op);         // 等待几秒以后就往下执行了，代表map任务超时
    ...
    if (!map->finishedMapTask.count(map->runningMapWork[map->curMapIndex])) {
        ...
        const char* text = map->runningMapWork[map->curMapIndex].c_str();  // 这钟方式加入list后不会变成空字符串  // text就是文件名
        map->m_list.push_back((char*)text);                                // 将该任务重新加入工作队列
        map->curMapIndex++;
...
void Master::waitMap(string filename) {
    m_assign_lock.lock();
    runningMapWork.push_back(
        string(filename));  // 将分配出去的map任务加入正在运行的工作队列
    m_assign_lock.unlock();
    pthread_t tid;
    pthread_create(&tid, NULL, waitMapTask,
...
void* Master::waitReduceTask(void* arg) {
    ...
    if (!reduce->finishedReduceTask.count(
            reduce->runningReduceWork[reduce->curReduceIndex])) {
        for (auto a : reduce->m_list)
...
bool Master::Done() {
    ...
    int len =
        finishedReduceTask
            .size();  // reduce的hashmap若是达到reduceNum，reduce任务及总任务完成
    m_assign_lock.unlock();
